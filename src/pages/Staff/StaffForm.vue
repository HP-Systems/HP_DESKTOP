...
import { registerStaff, updateStaff } from './StaffRequest';
import SelectInput from '@/components/SelectInput.vue';
let loading = ref(false);
const title = ref('Agregar Personal');
// eslint-disable-next-line no-unused-vars
const emit = defineEmits(['newStaff', 'closeModal', 'editStaff']);
...
onMounted(() => {
    ...
    Object.keys(props.staff).forEach((key) => {
        form.value[key] = props.staff[key];
    });
    if (props.staff.rol === 'Recepcionista') {
            form.value.rol = 2;
        }
        if (props.staff.rol === 'Limpieza') {
            form.value.rol = 3;
        }
})
...
const submit = () => {
    ...
    try{
        loading.value = true;
        if (props.action === 1) {
            registerStaff(form.value).then((response) => {
                loading.value = false;
                emit('closeModal');
                form.value.Personal_id = response.data.Personal_id;
        ...
        if (props.action === 2) {
            loading.value = true;
            console.log('props.staff.Personal_id', props.staff.Personal_id);
            updateStaff(form.value, props.staff.Personal_id).then((response) => {
                loading.value = false;
                emit('closeModal');
                form.value.Personal_id = props.staff.Personal_id;
                form.value.User_id = props.staff.User_id;
...
<template>
    ...
    <div class="flex flex-col items-center justify-center w-full mt-5">
        ...
        <div class="w-5/6 columns-1">
            <div class="w-full p-2">
                <SelectInput label="Rol:" id="rol" class="w-full mt-1 mr-6 block-sm" v-model="form.rol" :data="roles" :error="errors && errors.rol && errors.rol[0] !== null ? errors.rol[0] : ''"/>
            </div>
        </div>
